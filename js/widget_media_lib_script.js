...
var targetID;
  jQuery('.upload_image_button').live('click', function( event ){
    ...
    var startStr = str.indexOf(res); 
    var startStrText = str.substr(0, startStr);
    var finalURL = startStrText + "-65x65" + res;
	var http = new XMLHttpRequest();
	http.open('HEAD', finalURL, false);
	http.send();
	if (http.status == '404'){
		finalURL = attachment.url;
	}
		
	  jQuery('#'+targetID).val(finalURL);
	  jQuery('#'+targetID+'-preview').attr('src',finalURL);
    });
